...
def run_experiment(target, data, device, model_savepath=None, sample_ind=0, run_id=0):
    ...
    low_tests = data['low_test']
    high_tests = data['high_test']

    epochs = data['epochs']

    train_dataset = Dataset_NLP(num_samples=num_train_samples, num_cats=num_cats, low=low_train, high=high_train, target=target, num_additional_node=num_additional_node, num_query_cats=num_query_cats)
    val_dataset =  Dataset_NLP(num_samples=num_test_samples, num_cats=num_cats, low=low_train, high=high_train, target=target, num_additional_node=num_additional_node, num_query_cats=num_query_cats)
    test_datasets = [Dataset_NLP(num_samples=num_test_samples, num_cats=num_cats, low=low_test, high=high_test, target=target, num_additional_node=num_additional_node, num_query_cats=num_query_cats, reject_low=low_train, reject_high=high_train)
                    for (low_test, high_test) in zip(low_tests, high_tests)]
    
    n = train_dataset.list_size - num_additional_node # number of elements in the tokenized sequence excluding scratchpad
    ...
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=shuffling, variable_length=False)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False, variable_length=False)
    test_loaders = [DataLoader(test_dataset, batch_size=batch_size, shuffle=False, variable_length=False)
                    for test_dataset in test_datasets]

    model_s = Transformer(in_dim=in_dim_s, embed_dim=embed_dim, out_dim=out_dim, num_heads=num_heads, num_layers=num_layers, num_cats=num_cats,
                        mlp_hidden_dim=mlp_hidden_dim, mlp_num_layers=mlp_num_layers, positional=False, pos_dim=pos_enc_base.size(1)).to(device)
    model_p = Transformer(in_dim=in_dim_p, embed_dim=embed_dim, out_dim=out_dim, num_heads=num_heads, num_layers=num_layers, num_cats=num_cats,
    ...
    for epoch in range(epochs):
        ...
        for (x, y) in train_loader:
            ...
            x_app = x #append_positional_encoding(x, pos_enc_base)
        ...
        if epoch % 10 == 0:
            with torch.no_grad():
                ...
                test_losses_s, test_losses_p = [], []
                for i, test_loader in enumerate(test_loaders):
                    test_loss_s, test_loss_p = 0, 0
                    ...
                    test_loss_s /= len(test_loader)
                    test_loss_p /= len(test_loader)
                    test_losses_s.append(test_loss_s)
                    test_losses_p.append(test_loss_p)

                train_loss_s /= len(train_loader)
                train_loss_p /= len(train_loader)

                str_loss_p = "/".join([f"{loss:.4e}" for loss in test_losses_p])
                str_loss_s = "/".join([f"{loss:.4e}" for loss in test_losses_s])
                ...
                if epoch == epochs-1:
                    final_losses_s = [train_loss_s] + [val_loss_s] + test_losses_s
                    final_losses_p = [train_loss_p] + [val_loss_p] + test_losses_p
...
if __name__ == '__main__':
    ...
    num_cats = data['num_cats']
    num_query_cats = data['num_query_cats'] if 'num_query_cats' in data else "unspecified"
    low_train = data['low_train']
    high_train = data['high_train']
